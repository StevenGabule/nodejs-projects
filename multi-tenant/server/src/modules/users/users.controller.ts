...
import { assignRoleUser as assignRoleToUser, createUser, getUserByEmail, getUsersByApplication } from './users.services';
...
export async function loginHandler(request: FastifyRequest<{Body: LoginBody}>, reply: FastifyReply) {
	...
	const token = jwt.sign({id: user.id, email, applicationId, scopes: user.permissions}, "secret")
	return {token}
}
